...
void createDict(nodePointer dict[N])
{
	...
	string eng, ukr;
	int freq, i;
	cout << "Введіть словник('0' - вихід)" << endl;
	while (true) {
		cout << "Введіть англійське слово" << endl;
		cin >> eng;
		if (eng[0] == '0') {
			break;
		}
		cout << "Введіть український переклад" << endl;
		cin >> ukr;
		cout << "Введіть частоту використання" << endl;
		cin >> num;
		freq = atoi(num);
...
void main()
{
	...
	initArr(dict);
	createDict(dict);
	cout << "Введіть речення англійською мовою" << endl;
	string str;
	cin.ignore();
	getline(cin, str);
	translate(dict, str);
